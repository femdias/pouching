...
*--------------------------*
* ADDING MORE CONTROLS, number of workers and managers pouched)
*--------------------------*
	...
	esttab c5_spv_4_alt c5_spvs_4_alt c5_epvs_4_alt c5_spv_5_alt c5_spvs_5_alt c5_espv_5_alt c6_spv_6_alt c6_spvs_6_alt c6_epvs_6_alt using "${results}/alternatives_alt.tex", booktabs  /// 
		replace compress noconstant nogap collabels(none) label /// 
		mlabel("Mgr-Mgr" "Mgr-Wkr" "Wkr-Mgr" "Mgr-Mgr" "Mgr-Wkr" "Wkr-Mgr" "Mgr-Mgr" "Mgr-Wkr" "Wkr-Mgr") ///
		refcat(d_size_w_ln "\midrule", nolabel) ///
		mgroups("Prediction 4" "Prediction 5" "Prediction 6",  /// 
		...
		*** Different models
	esttab c5_spv_4_alt c5_spv_5_alt c6_spv_6_alt c5_spvs_4_alt c5_spvs_5_alt c6_spvs_6_alt c5_epvs_4_alt c5_espv_5_alt c6_epvs_6_alt using "${results}/alternatives_alt.tex", booktabs  /// 
		replace compress noconstant nogap collabels(none) nomtitles label /// 
		refcat(d_size_w_ln "\midrule \textbf{Destination Firm}" ///
			o_size_w_ln "\textbf{Origin firm}" ///
			lnraid "\textbf{Raided workers}", nolabel) ///
		mgroups("Manager to Manager" "Manager to Non-Manager" "Non-Manager to Manager",  /// 
		pattern(1 0 0 1 0 0 1 0 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///    
		keep(d_size_w_ln d_growth_w o_size_w_ln o_avg_fe_worker  c.o_size_w_ln#c.o_avg_fe_worker  lnraid rd_coworker_fe  c.rd_coworker_fe#c.lnraid ) ///
		order(d_size_w_ln d_growth_w o_size_w_ln o_avg_fe_worker c.o_size_w_ln#c.o_avg_fe_worker lnraid rd_coworker_fe c.rd_coworker_fe#c.lnraid) ///
		coeflabels(d_size_w_ln "Size (ln employment)" ///
			d_growth_w "Employment growth rate" ///
			o_size_w_ln "Size (ln employment)" ///
			o_avg_fe_worker "Avg worker ability (AKM FE)" ///
			c.o_size_w_ln#c.o_avg_fe_worker "Size (ln employment) $\times$ Avg worker ability" ///
			rd_coworker_fe "Avg ability (AKM FE)" ///
			lnraid "Quantity (ln \# raided workers)" ///
			c.rd_coworker_fe#c.lnraid "Avg ability $\times$ quantity") ///
		cells(b(star fmt(3)) se(par fmt(3))) ///    
		stats(N r2 pred, fmt(0 3 0) label(" \\ Obs" "R-Squared" "Prediction")) ///
		obslast nolines  starlevels(* 0.1 ** 0.05 *** 0.01) ///
		indicate("\\ \textbf{Manager controls} \\ Manager salary at origin = pc_wage_o_l1" ///
