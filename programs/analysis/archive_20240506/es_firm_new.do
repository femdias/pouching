...
// Purpose: Event study around poaching events (directors)

*--------------------------*
* BUILD
*--------------------------*

// temporary: selecting events where a director was a poached and reemployed as a director

	use "output/data/cowork_panel_m_rs_dir", clear

	// identifying the events when a director was poached
	
	sort event_id cpf ym
	bysort event_id cpf: gen dir_poached = (dir[_n-1] == 1) & (plant_id[_n-1] == o_plant) & (plant_id == d_plant)

	collapse (max) dir_poached, by(event_id)
	
	save "temp/dir_poached", replace
	
	// saving temp file do evt_panel_m_rs_dir, keeping only the events where a director was poached
	
	use "output/data/evt_panel_m_rs_dir", clear
	merge m:1 event_id using "temp/dir_poached"
	keep if dir_poached == 1
	drop if d_plant == 4654447000488
	save "output/data/evt_panel_m_rs_dir_CORRETO", replace
...
// 4. how many of these hires are from the origin firm? (relative to number of monthly hires)	
	...
	tsline share, recast(connect) mcolor(navy) m(Oh) ///
		graphregion(lcolor(white)) plotregion(lcolor(white)) ///
		xtitle("Relative month (manager poached at t=-1, starts at t=0)") xlabel(-12(1)12) xline(-1, lpattern(dash) lcolor(gs13)) ///
		ytitle("Share of new hires (monthly)" "from same firm as poached manager") ylabel(, grid glpattern(dash) glcolor(gs13*.3) gmax)	///
		name(d_h_emp_o_share, replace)
