...
// combining events and keeping what we need
	...
	*/
	...
	// Defining industry global
	global industries manu ag_mining utilities construction ret transportation finance serv education government health other

	// Creating industries string variable
	foreach var in 1 2 {
	    gen industry`var' = ""
	    foreach ind of global industries {
	    	di "`ind'"
		replace industry`var' = "`ind'" if `ind'`var' == 1
	    }
	}
	
	// Within vs Across Industries moviments
	gen within_industry1 = industry1 == industry2
	gen within_industry2 = within_industry1
	
	*tab within_industry1
	/*
	within_indu |      Freq.     Percent        Cum.
	------------+-----------------------------------
		  0 |      1,622       28.35       28.35
		  1 |      4,100       71.65      100.00
	------------+-----------------------------------
	      Total |      5,722      100.00

	*/
	
	*rename industry1 ind_dest
	*rename industry2 ind_orig
	*tab ind_orig ind_dest 
	*tab ind_orig ind_dest, row nofreq
	...
	*replace pc_wage_resid1 = pc_wage_resid1 + _b[_cons]
	...
	pc_wage pc_wage_lvl pc_wage_w pc_wage_resid pc_age pc_exp pc_fe within_industry, ///
	...
	la var within_industry "Destination and Origin firms within the same industry"
