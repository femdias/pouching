...
// main line
	...
	* we need to bind to get some selections of sample
	...
	*append using `emp'
	...
	g zeroraid = (d_h_emp_o==0)
	g zerohire = (d_h_emp==0)
	g nonzeroraid = (d_h_emp_o>0 & d_h_emp_o!=.)
	g nonzerohire = (d_h_emp_o>0 & d_h_emp!=.)
	...
	eststo mainnum: reghdfe d_h_emp_o $evt_vars if pc_ym >= ym(2010,1) & pc_ym <= ym(2016,12) ///
		& ym_rel >= -9, absorb(event_id d_plant) vce(cluster event_id)
...
// figure
	...
	coefplot (main, recast(connected) keep(${evt_vars}) msymbol(T) mcolor(emerald) mlcolor(emerald) msize(medium) ///
		 ...
		 ylabel(-.02(.02).1) name(spv, replace)
	...
	coefplot (mainnum, recast(connected) keep(${evt_vars}) msymbol(T) mcolor(emerald) mlcolor(emerald) msize(medium) ///
		 ...
		 xlabel(1 "-9" 4 "-6" 7 "-3" 10 "0" 13 "3" 16 "6" 19 "9" 22 "12") ///
		 xtitle("Months relative to poaching event") ///
		 ytitle("Average number of new hires" "from the same firm as poached worker") ///
