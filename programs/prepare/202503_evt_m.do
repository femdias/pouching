...
foreach ym of numlist 601/611 613/623 625/635 637/647 649/659 661/671 673/683 { // note: we can't identify m1 poaching events yet
	...
	save "${temp}/202503_plantsize_m_`ym'", replace
	...
	merge m:1 plant_id ym using "${temp}/202503_plantsize_m_`ym'", keepusing(n_emp_lavg)
	drop _merge
...
// II. dealing with multiple poaching events in the same month and same origin/destination	
	...
	// if an origin firm is having its workers poached by more than 1 destination firm, select just one destination
		...
		tab n_dest
			...
			preserve
				...
				compress
				save "${temp}/202503_o_plant_multiple", replace
	...
	// if a destination firm is poaching workers from more than 1 origin firm, select just one origin
		...
		tab n_orig
			...
			preserve
				...
				compress
				save "${temp}/2025_d_plant_multiple", replace
	...
	compress
	save "${temp}/202503_eventid", replace
		
	// now, merging this back with the complete list of poaching events
	
	use "${data}/202503_evt_m", clear
	merge m:1 o_plant d_plant pc_ym using "${temp}/202503_eventid"
	keep if _merge == 3
	drop _merge
		...
		merge m:1 o_plant pc_ym using "${temp}/202503_o_plant_multiple"
		replace multiple = 1 if _merge == 3
		drop _merge
		
		merge m:1 d_plant pc_ym using "${temp}/202503_d_plant_multiple"
		replace multiple = 1 if _merge == 3
		drop _merge
...
foreach ym of numlist 601/611 613/623 625/635 637/647 649/659 661/671 673/683 {
	...
	rm "${temp}/202503_plantsize_m_`ym'.dta"
	rm "${temp}/202503_evt_m_`ym'.dta"
	
}

rm "${temp}/202503_o_plant_multiple.dta"
rm "${temp}/202503_d_plant_multiple.dta"
rm "${temp}/202503_eventid.dta"
