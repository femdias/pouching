...
*foreach e in dir dir5 spv emp { // STILL NEED TO RUN THIS FOR DIR5 SPV --- NOTE THAT THIS WON'T WORK FOR EMP!	NEED TO MODIFY LINE 196
...
// first: create an empty panel around the t=0 (when poaching takes place)
	...
	save "output/data/evt_panel_m_rs_`e'", replace
...
// second: plant-month variables for the destination plant
	...
	// listing plants and months i'm interested in
		...
		use "output/data/evt_panel_m_rs_`e'", clear
...
// third: plant-month variables in the destination plant FROM ORIGIN PLAN
	...
	use "output/data/cowork_panel_m_rs_`e'", clear
...
// fourth: similar to third, but just for the poached individuals
	...
	bysort event_id cpf: gen pc = ym_rel == 0 & (`e'[_n-1] == 1) & (plant_id[_n-1] == o_plant) & (plant_id == d_plant)
...
// fourth: merging everything with the main data set
	...
	// saving
	save "output/data/evt_panel_m_rs_`e'.dta", replace
