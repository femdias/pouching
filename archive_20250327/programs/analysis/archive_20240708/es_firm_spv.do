...
// Purpose: Event study around poaching events (supervisors)

*--------------------------*
* BUILD
*--------------------------*

// auxiliary data set: identifying events where the poached supervisor stays in the new firm

	use "output/data/cowork_panel_m_rs_spv", clear
	
	// identifying the poached individuals among the coworkers
	
	bysort event_id cpf: gen pc = ym_rel == 0 & (spv[_n-1] == 1) & (plant_id[_n-1] == o_plant) & (plant_id == d_plant)
	...
	// saving
	save "temp/total_emp_d_plant_spv", replace
...
// all figures
	...
	use "output/data/evt_panel_m_rs_spv", clear
	...
	// sample restrictions
		...
		*drop if event_id == 100 // hire thousands of employees; probably tranfers or reporting issue
		...
		merge m:1 event_id using "temp/total_emp_d_plant_spv"
		drop if _merge == 2
		drop _merge
	...
	*/
	...
	tsline `var', recast(connect) mcolor(gs8) lcolor(gs8) m(Oh) ///
		...
		graph export "output/results/es_spv_`var'.pdf", as(pdf) replace
