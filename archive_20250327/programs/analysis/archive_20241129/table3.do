...
foreach e in spv_dir emp_placebo spv dir emp spv_emp emp_placebo_sample {
	

	display "`e'"
	use "${temp}/table3_`e'", clear
	
	// in 2 obs, this number is > 1 due to the fact that we only included readmission as new hire
	...
	local missingvars pc_exp_ln pc_fe pc_wage_o_l1 fe_firm_d fe_firm_o ///
			  d_size_ln d_growth_winsor turnover ratio_cw_new_hire rd_coworker_fe
	...
	*eststo clear
		...
		eststo c4_`e': reg pc_wage_d d_size_ln d_size_ln_m ratio_cw_new_hire ratio_cw_new_hire_m ///
				`ctrl' `ctrl_m' if pc_ym >= ym(2010,1), rob
		...
		eststo c6_`e': reg pc_wage_d d_size_ln d_size_ln_m d_growth_winsor d_growth_winsor_m ///
				turnover turnover_m ratio_cw_new_hire ratio_cw_new_hire_m rd_coworker_fe rd_coworker_fe_m ///
				`ctrl' `ctrl_m' if pc_ym >= ym(2010,1), rob
  ...
  esttab c1_`e' c2_`e' c3_`e' c4a_`e' c6a_`e' c7a_`e' /// 
    using "${results}/table3_`e'.tex", tex /// 
    ...
    keep(d_size_ln d_growth_winsor turnover d_raid_individual rd_coworker_fe) ///
