...
// all figures
	...
	// outcome variables
		...
		* gen d_emp = d_emp_dir + d_emp_spv + d_emp_emp // already in data set
		...
		/*
					...
					tab d_h_spv_o
					tab d_h_emp_o
					tab d_h_o_pc // not found
					tab d_h_dir_o_pc
					tab d_h_spv_o_pc
					tab d_h_emp_o_pc
					
					gen teste1 = d_h_dir_o == d_h_dir_o_pc 
					tab teste1 // always identical
					
					gen teste2 = d_h_spv_o == d_h_spv_o_pc 
					tab teste2 // always identical
					
					gen teste3 = d_h_emp_o == d_h_emp_o_pc 
					tab teste3 // always identical
			*/		
	...
	*/
	...
	tsline `var', recast(connect) mcolor(gs8) lcolor(gs8) m(Oh) ///
		...
		graph export "output/results/es_dir_`var'_new.pdf", as(pdf) replace
...
// all figures
	...
	*/
	...
	local td_emp			""Number of employees (monthly)""
	local td_h_d_emp		""Share of new hires (monthly)" "relative to monthly employment""
	local td_h_o_pc 		""Number of new poached supervisors (monthly)" "from same firm as poached supervisor""
	local td_h_o_sanspc		""Number of new hires (monhtly)" "from same firm as poached supervisor""
	local td_h_o_d_h_sanspc		""Share of new hires (monthly)" "from same firm as poached supervisor""
	local td_h_o_d_h_sanspc_0	""Share of new hires (monthly)" "from same firm as poached supervisor""	
	local td_emp_o_pc		""Cumulative number of directors (monthly)" "from same firm as poached supervisor""
	local td_emp_o_sanspc		""Cumulative number of employees (monthly)" "from same firm as poached supervisor""
	local td_emp_o_d_emp_sanspc	""Cumulative share of employees (monthly)" "from same firm as poached supervisor""
	local tcuml_d_h_o_d_h_sanspc	""Share of cumulative hires" "from same firm as poached supervisor""
	local tcuml_d_h_o_d_h_sanspc_0 	""Share of cumulative hires" "from same firm as poached supervisor""
	...
	tsline `var', recast(connect) mcolor(gs8) lcolor(gs8) m(Oh) ///
		graphregion(lcolor(white)) plotregion(lcolor(white)) ///
		xtitle("Relative month (supervisor poached at t=-1, starts at t=0)") xlabel(-12(3)12) xline(-1, lpattern(dash) lcolor(gs13)) ///
		ytitle(`t`var'') ylabel(`l`var'', grid glpattern(dash) glcolor(gs13*.3) gmax)
	
		graph export "output/results/es_spv_`var'_new.pdf", as(pdf) replace
	...
	tsline d_emp, recast(connect) mcolor(gs1) m(Oh) lcolor(gs1) ///
		...
		graph export "output/results/es_spv_emp_h.pdf", as(pdf) replace
...
// all figures
	...
	*/
	...
	local td_emp			""Number of employees (monthly)""
	local td_h_d_emp		""Share of new hires (monthly)" "relative to monthly employment""
	local td_h_o_pc 		""Number of new poached employees (monthly)" "from same firm as poached employee""
	local td_h_o_sanspc		""Number of new hires (monhtly)" "from same firm as poached employee""
	local td_h_o_d_h_sanspc		""Share of new hires (monthly)" "from same firm as poached employee""
	local td_h_o_d_h_sanspc_0	""Share of new hires (monthly)" "from same firm as poached employee""	
	local td_emp_o_pc		""Cumulative number of directors (monthly)" "from same firm as poached employee""
	local td_emp_o_sanspc		""Cumulative number of employees (monthly)" "from same firm as poached employee""
	local td_emp_o_d_emp_sanspc	""Cumulative share of employees (monthly)" "from same firm as poached employee""
	local tcuml_d_h_o_d_h_sanspc	""Share of cumulative hires" "from same firm as poached employee""
	local tcuml_d_h_o_d_h_sanspc_0 	""Share of cumulative hires" "from same firm as poached employee""
	...
	tsline `var', recast(connect) mcolor(gs8) lcolor(gs8) m(Oh) ///
		...
		graph export "output/results/es_emp_`var'_new.pdf", as(pdf) replace
