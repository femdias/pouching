...
// combining events and keeping what we need
	...
	merge m:1 spv event_id using "${temp}/eventlist", keep(match)
				...
				// TEMPORARY: keeping only cases where there is no joint movement
				
				// creating new event_id
				tostring event_id, gen(event_id_str) format(%09.0f) force
				gen evt = ""
				replace evt = "spv" if spv == 1 | spv == 2
				replace evt = "emp" if spv == 3
				gen evtid = evt + "_" + event_id_str
				
				drop _merge
				merge m:1 evtid using "${temp}/evt_m_nojoint"
				keep if _merge == 3
				drop _merge
...
*--------------------------*	
	...
	coefplot (main, recast(connected) keep(${evt_vars}) msymbol(T) mcolor(black%60) mlcolor(black%80) msize(medium) ///
		 ...
		 graph export "${results}/es_baseline_temp_20250210.pdf", as(pdf) replace
