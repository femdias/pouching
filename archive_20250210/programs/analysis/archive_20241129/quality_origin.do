...
/*
	...
	cap rm "${temp}/quality_origin_`e'.dta"
	...
	forvalues ym=612/683 { 
		...
		if _N >= 1 {
			...
			cap append using "${temp}/quality_origin_`e'"
			save "${temp}/quality_origin_`e'", replace
		}
		}
	}
...
keep if type_spv == 1
save "${temp}/quality_origin_final_spv", replace

// dir -- > spv
use "${temp}/quality_origin_dir", clear
keep if type_spv == 1
save "${temp}/quality_origin_final_dir", replace

// emp --> spv
use "${temp}/quality_origin_emp", clear
keep if type_spv == 1
save "${temp}/quality_origin_final_emp", replace

// emp --> emp
use "${temp}/quality_origin_emp", clear
keep if type_emp == 1
save "${temp}/quality_origin_final_emp_placebo", replace
	
// emp --> emp (placebo)
use "${temp}/quality_origin_emp", clear
merge m:1 event_id using "${temp}/events_raided_moveable_emp_placebo", keep(match) nogen // subset of events here -- comes from quality_origin_sel_evts
keep if type_emp == 1
save "${temp}/quality_origin_final_emp_placebo_sample", replace

// spv --> spv + dir --> spv	
use "${temp}/quality_origin_final_spv", clear
append using "${temp}/quality_origin_final_dir"
save "${temp}/quality_origin_final_spv_dir", replace

// spv -- > emp
use "${temp}/quality_origin_spv", clear
keep if type_emp == 1
save "${temp}/quality_origin_final_final_spv_emp", replace
	
	
	
	foreach e in spv dir emp spv_dir emp_placebo spv_emp {
	
	use "${temp}/quality_origin_final_`e'", clear
	...
	// CDF: raided new hires vs non-raided leftover people
		...
		distplot fe_worker, over(group) xtitle("Worker FE") ytitle("Cumulative Probability") ///
			...
			graph export "${results}/quality_origin_`e'.pdf", as(pdf) replace
			graph export "${results}/quality_origin_`e'.png", as(png) replace
...
distplot fe_worker_winsor, over(group) xtitle("Worker Quality (AKM worker fixed effect)") ///
    ...
    legend(region(lstyle(none)))
			...
			graph export "${results}/quality_origin_winsor_`e'.pdf", as(pdf) replace
			graph export "${results}/quality_origin_winsor_`e'.png", as(png) replace
	...
	// CDF: by group instead of by individual
		...
		distplot fe_worker, over(group) xtitle("Worker FE") ytitle("Cumulative Probability") ///
			...
			graph export "${results}/quality_origin_`e'_by_evt_id.pdf", as(pdf) replace
			graph export "${results}/quality_origin_`e'_by_evt_id.png", as(png) replace
